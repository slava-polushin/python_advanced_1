...
class Cars(Base):
    ...
    created_at = mapped_column(DateTime(True), server_default=text('now()'), comment='Дата создания записи')
    modified_at = mapped_column(DateTime(True), server_default=text('now()'), server_onupdate=text('now()'), comment='Дата изменения записи')

    car_drivers: Mapped[List['CarDrivers']] = relationship('CarDrivers', uselist=True, back_populates='car')
    car_status: Mapped[List['CarStatus']] = relationship('CarStatus', uselist=True, back_populates='car')
    order_status: Mapped[List['OrderStatus']] = relationship('OrderStatus', uselist=True, back_populates='car')
...
class Clients(Base):
    ...
    comment = mapped_column(Text, comment='Комментарий')
    created_at = mapped_column(DateTime(True), server_default=text('now()'), comment='Дата создания записи клиента')
    modified_at = mapped_column(DateTime(True), server_default=text('now()'), server_onupdate=text('now()'), comment='Дата изменения записи клиента')
...
class Drivers(Base):
    ...
    driver_license = mapped_column(String, nullable=False, comment='Номер водительского удостоверения')
    comment = mapped_column(Text, comment='Комментарий')
    created_at = mapped_column(DateTime(True), server_default=text('now()'), comment='Дата создания записи клиента')
    modified_at = mapped_column(DateTime(True), server_default=text('now()'), server_onupdate=text('now()'), comment='Дата изменения записи клиента')

    car_drivers: Mapped[List['CarDrivers']] = relationship('CarDrivers', uselist=True, back_populates='driver')
...
class CarDrivers(Base):
    ...
    driver: Mapped[Optional['Drivers']] = relationship('Drivers', back_populates='car_drivers')


class Orders(Base):
    __tablename__ = 'orders'
    __table_args__ = (
        ForeignKeyConstraint(['client_id'], ['clients.client_id'], name='orders_client_id_fkey'),
    ...
    client_id = mapped_column(BigInteger, nullable=False, comment='Идентификатор пользователя')
    start_address = mapped_column(String, nullable=False, comment='Адрес начала поездки')
    start_latitude = mapped_column(Numeric(6, 5), comment='широта точки старта')
    start_longitude = mapped_column(Numeric(6, 5), comment='долгота точки старта')
    finish_address = mapped_column(String, comment='Адрес завершения поездки')
    finish_latitude = mapped_column(Numeric(6, 5), comment='широта точки финиша')
    finish_longitude = mapped_column(Numeric(6, 5), comment='долгота точки финиша')
    price = mapped_column(Numeric(18, 2), comment='цена поездки')
    comment = mapped_column(Text, comment='Комментарий')
    baby_chair_fl = mapped_column(Boolean, comment='признак необходимости детского кресла')
    created_at = mapped_column(DateTime(True), server_default=text('now()'), comment='Дата создания заказа')
    modified_at = mapped_column(DateTime(True), server_default=text('now()'), server_onupdate=text('now()'), comment='Дата изменения заказа')

    client: Mapped['Clients'] = relationship('Clients', back_populates='orders')
    car_status: Mapped[List['CarStatus']] = relationship('CarStatus', uselist=True, back_populates='order')
    order_status: Mapped[List['OrderStatus']] = relationship('OrderStatus', uselist=True, back_populates='order')


class CarStatus(Base):
    __tablename__ = 'car_status'
    __table_args__ = (
        ForeignKeyConstraint(['car_id'], ['cars.car_id'], name='car_status_car_id_fkey'),
        ForeignKeyConstraint(['order_id'], ['orders.order_id'], name='car_status_order_id_fkey'),
    ...
    id = mapped_column(BigInteger, comment='Идентификатор записи состояния автомобиля')
    car_id = mapped_column(BigInteger, nullable=False, comment='Идентификатор автомобиля')
    status = mapped_column(String, nullable=False, comment='Статус автомобиля, может быть равен: {free, busy, broken, driver_missing}')
    current_latitude = mapped_column(Numeric(6, 5), comment='Широта текущего местоположения')
    current_longitude = mapped_column(Numeric(6, 5), comment='Долгота текущего местоположения')
    order_id = mapped_column(BigInteger, comment='Заказ, на который назначен автомобиль на текущий момент (может быть пустым)')
    comment = mapped_column(Text, comment='Комментарий')
    created_at = mapped_column(DateTime(True), server_default=text('now()'), comment='Дата создания исторической записи заказа')
...
class OrderStatus(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(['car_id'], ['cars.car_id'], name='order_status_car_id_fkey'),
        ForeignKeyConstraint(['order_id'], ['orders.order_id'], name='order_status_order_id_fkey'),
    ...
    id = mapped_column(BigInteger, comment='Идентификатор записи истории заказа')
    order_id = mapped_column(BigInteger, nullable=False, comment='Идентификатор заказа')
    status = mapped_column(String, nullable=False, comment='Статус заказа, может быть равен: {created, car_assigned, trip_started, trip_finished, cancelled}')
    car_id = mapped_column(BigInteger, comment='Идентификатор автомобиля')
    comment = mapped_column(Text, comment='Комментарий')
    ...
    car: Mapped[Optional['Cars']] = relationship('Cars', back_populates='order_status')
    order: Mapped['Orders'] = relationship('Orders', back_populates='order_status')
