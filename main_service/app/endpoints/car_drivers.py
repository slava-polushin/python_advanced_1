...
@router.get("/driver_via_car/{car_id}", response_model=CarDriver)
...
@router.get("/car_via_driver/{driver_id}", response_model=CarDriver)
def read_car_via_driver(driver_id: int, db: Session = Depends(get_db)):
    fromdate=date.today()
    db_client = get_car_for_driver(db, driver_id=driver_id,fromdate=fromdate)
